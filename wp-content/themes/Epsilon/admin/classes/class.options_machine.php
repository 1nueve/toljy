...
class Options_Machine {
			...
			switch ( $value['type'] ) {
				...
				/*****ADDED BY WPEXPLORER*****/
					...
					case 'slider_testimonials':
						...
						$output .= '</ul>';
						$output .= '<a href="#" class="button testimonial_add_button">Add New Testimonial</a></div>';
	...
	public static function optionsframework_slider_function($id,$std,$oldorder,$order,$int){
		...
		// ADDED BY WPEXPLORER
		$slider .= '<label>Enter in a video URL that is compatible with WordPress\'s built-in oEmbed feature. (Use Instead of Image)</label>';
		$slider .= '<input class="slide of-input" name="'. $id .'['.$order.'][video]" id="'. $id .'_'.$order .'_slide_video" value="'. $val['video'] .'" />';
		// ADDED BY WPEXPLORER
	...
	/**
	 * Drag and drop testimonials manager
	 *
	 * @uses get_option()
	 *
	 * @access public
